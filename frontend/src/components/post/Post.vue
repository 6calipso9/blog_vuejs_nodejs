  ...
  <div class="text-center">
    ...
    <div class="card mx-auto" style="width: 40rem;">
      <div class="card-header text-center">
        <h6>Write comment</h6>
        <div v-show="sucessAdded" class="alert alert-success">Comment added!</div>
      </div>
      <div class="card-body">
        <textarea
          class="form-control"
      ...
      <div class="card-body">
        <div v-for="commentar in comments" :key="commentar._id" class="media-body">
          ...
          <div class="alert alert-secondary text-left" role="alert">{{commentar.text}}</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getComments() {
      axios.get(`${server.baseURL}/posts/${this.id}/comments`).then(data => {
        (this.comments = data.data.comments), console.log(this.comments);
      });
    },
    getPost() {
      axios.get(`${server.baseURL}/posts/${this.id}`).then(data => {
    ...
    __submitCommentToServer(data) {
      this.$http
        .post(`${server.baseURL}/posts/${this.id}/comments`, data, headers)
        .then(data => {
          console.log(data);
          this.sucessAdd = true;
