<template>
  <div>
    <div class="card mx-auto" style="width: 30rem;">
      ...
      <div class="card-body">
        <div
          v-show="errorNameOrPass"
          class="alert alert-warning"
          id="errorMSG"
        >Email or pass not actual</div>

        <div v-show="errorServer" class="alert alert-danger">Server error</div>
        <div v-show="sucesslogin" class="alert alert-success">Sucess login</div>
        ...
        <form id="create-post-form" @submit.prevent="LoginUser">
          ...
          <div class="form-group col-md-4 pull-right">
            <button class="btn btn-success" type="submit" id="btnLoginGo">Login</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    __submitToServer(data) {
      this.$http
        .post(`${server.baseURL}/session`, data)
        .then(response => {
